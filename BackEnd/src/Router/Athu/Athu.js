...
router.post('/login', async (req, res) => {
    try {
      ...
      if (result.length > 0) {
        ...
        {
          console.log("Sucesss")
          const tableListName = result[0].email.replace('@', '_').replace('.', '_') + '_list';
          const tableCardName = result[0].email.replace('@', '_').replace('.', '_') + '_card';

          const token = jwt.sign(
            { id: result[0].id ,name : result[0].firstName+result[0].lastName ,TableListName: tableListName, TableCardName: tableCardName}, process.env.TOKEN_KEY, {
            expiresIn: 3600, // Expires in 1 hour
          });
          console.log(" --- username and password is correct ---")

          res.status(200).json({ status: true, message: 'Sucesss', name : result[0].firstName+result[0].lastName , token:token});
        }
        else
        {
